...
excel_file = 'C:/Spyder projects/PVML/30.5.2019_ML_QK_TF_NN.xls'
...
#W10 = tf.Variable(np.random.randn(), name = "W10")
...
# Hypothesis 
#y_pred = tf.multiply(X, W1)+ tf.multiply(X**2,W2)+ tf.multiply(X**4,W3)+ b
#y_pred = tf.multiply(X, W1)+ tf.multiply(X**2,W2)+ tf.multiply(X**3,W3)+ tf.multiply(X**4,W4)+ tf.multiply(X**5,W5)+ tf.multiply(X**6,W6)+ tf.multiply(X**7,W7)+ tf.multiply(X**8,W8)  + b
...
with tf.Session() as sess: 
    ...
    for epoch in range(training_epochs): 
        ...
        # Displaying the result after every 500 epochs 
        if (epoch+1) % 500 == 0: 
